...
def extract_tts_in_window(chrom, batch):
    chromosome_number = chrom[3:]
    # Read in Windows and bam info
    # Define the directory containing the numpy arrays
    windows = pd.read_csv(f"/home/alextu/scratch/compute_windows/matched_windows/HG00268/{chrom}_matched_windows.csv")
    ...
    directory = f"/home/alextu/scratch/compute_windows/extracted_metrics/extracted_metrics_hg00268/chr14/{batch}"
        ...
        if filename.startswith(f"{chromosome_number}_") and filename.endswith("_negative.npy"):
            ...
            for index, row in relevant_windows_subset.iterrows():
                ...
                if combination_pos in processed_combinations_pos:
                    ...
                    extracted_metrics_df_row_pos['direction'] = row['direction']
            ...
            # Free memory
            del samples_metrics_pos
            del extracted_metrics_dfs
    ...
    filtered_extracted_metrics_df_pos.to_csv(f'/home/alextu/scratch/compute_windows/translocations_dfs/HG00268/tts_{chrom}_all_nonbstructures_negative_{batch}.csv', index=False)

#i = int(sys.argv[1])
for batch_num in batches:
    extract_tts_in_window(chromosomes, batch_num)
