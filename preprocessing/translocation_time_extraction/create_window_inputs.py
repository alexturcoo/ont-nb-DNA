...
def process_chromosome(chrom):
    print("Started")
    flowcell = "20210903_210825_21-lee-006_PCT0053_2-A1-D1"
    # Read non-B DNA windows
    windows_path_pos = '/home/alextu/scratch/nonb_db_preprocessing/non_overlapping_windows_0_based/pos'
    this_windows_path_pos = os.path.join(windows_path_pos, 'non_overlapping_windows_' + chrom + '+.csv')
    windows_per_chr_pos = pd.read_csv(this_windows_path_pos)
    windows_path_neg = '/home/alextu/scratch/nonb_db_preprocessing/non_overlapping_windows_0_based/neg'
    this_windows_path_neg = os.path.join(windows_path_neg, 'non_overlapping_windows_' + chrom + '-.csv')
    windows_per_chr_neg = pd.read_csv(this_windows_path_neg)
    # Extract the first 11 columns
    windows_per_chr = pd.concat([windows_per_chr_pos, windows_per_chr_neg], ignore_index=True)
    ...
    filtered_reads_path = '/home/alextu/scratch/compute_windows/reads_in_nonb_range/HG00268/20210903_210825_21-lee-006_PCT0053_2-A1-D1'
    ...
    for _, read in filtered_reads_per_chr_pos.iterrows():
        ...
        relevant_windows = windows_per_chr[(windows_per_chr["direction"] == 'same') & 
                                            (windows_per_chr['win_end'] >= read['mapped_start']) & 
    ...
    for _, read in filtered_reads_per_chr_neg.iterrows():
        # Filter windows to intersect with the read
        relevant_windows = windows_per_chr[(windows_per_chr["direction"] == 'opposite') & 
                                           (windows_per_chr['win_end'] >= read['mapped_start']) & 
                                           (windows_per_chr['win_start'] <= read['mapped_end'])]
    ...
    reads_in_windows_df_sorted.to_csv(f'/home/alextu/scratch/compute_windows/matched_windows/HG00268/20210903_210825_21-lee-006_PCT0053_2-A1-D1/{chrom}_matched_windows.csv', index=False)
